%semnal dreptunghiular x de perioada 40s
%durata 9s, factor de umplere
%9/T*100
T=40
w0=(2*pi)/T
t=0:0.1:100
...
x2=0
%for pentru calculul coeficientilor de la -50 la 50
...
for k=-N:N
    ...
    %semnalul afisat va fi suma modulelor coeficientilor
    x2=x2+real(C(k+N+1)*exp(1j*k*w0*t))
end

%se reprezinta graficul spectrului de amplitudine
subplot(2, 1, 1)
stem((-N:N), abs(C)); title('Spectru de amplitudini');
subplot(2, 1, 2); 

%se reprezinta semnalul initial
plot(t, x, '-')
hold on

%se reprezinta semnalul reconstruit
